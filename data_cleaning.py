...
# Ensure correct data types (datetime, numeric, and categorical columns)
def ensure_correct_data_types(df, date_columns, numeric_columns=None):
    ...
    # Ensure numeric columns are of the correct type (if provided)
    if numeric_columns:
        for column in numeric_columns:
            if column in df.columns:
                df[column] = pd.to_numeric(df[column], errors='coerce')  # Coerce invalid numbers to NaN
    
    # Convert categorical columns to 'category' dtype (optional, you can modify if needed)
...
# Convert dates in the dataframes:
...
# Ensure correct data types in both datasets (optional: specify numeric columns to enforce them)
#cash_request_data = ensure_correct_data_types(cash_request_data, cash_request_date_columns)
#fees_data = ensure_correct_data_types(fees_data, fees_data_date_columns)
...
def remove_nan(data_frame, col_):
    '''This function removes rows, but only rows where a specific column
       has NaN values. It also prints the number of rows removed and the
